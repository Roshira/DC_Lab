#include <cstdio>  // Замість stdio.h
#include <cstdlib> // Замість stdlib.h
#include <ctime>   // Замість time.h
#include <cmath>   // Замість math.h
#include <mpi.h>
...
double* &pProcResult, int &Size, int &RowNum) {
    ...
    // ПРИМІТКА: Цей блок if (ProcRank == 0) був відсутній у вашому коді, 
    // але він, ймовірно, потрібен, щоб уникнути race condition
    if (ProcRank == 0) { 
        if (Size < ProcNum) {
            printf("Size must be greater than number of processes! \n");
        }
...
int Size, int RowNum) {
    ...
    int PivotPos = 0; // Position of the pivot row in the process stripe
...
int main(int argc, char* argv[]) {
    ...
    // TEST 1-8
    for (int i = 0; i < 8; i++) {
        if (ProcRank == 0) { // Додано, щоб уникнути 8-микратного виводу
            printf("\nTest %d", (i + 1));
        }
